...
$Apache::ModuleDoc::VERSION = (qw$Revision: 1.2 $)[1];

my($ServerVersion) = SERVER_VERSION =~ m:Apache/(\S+):;
...
sub overrides {
    ...
    return(HTML::Entities::encode($retval), $override);
}
...
sub menu {
    ...
    print "<UL>\n";
    for my $cmd (@cmds) {
	my $text = HTML::Entities::encode($cmd);
	(my $name = $cmd) =~ s/[<>]/./g;
...
sub body {
	...
	print "Description: ", 
               HTML::Entities::encode($cmd_rec->errmsg), "<br>";
